...
DEFAULT_FAST = "20000"
DEFAULT_RATE = "200"
UNIT_OPTIONS = ("controller", "um", "degree")
STAGE_KIND_OPTIONS = ("linear", "rotation")
SHRC_UNIT_CODES = {"controller": "", "um": "U", "degree": "D"}
HITMV_LINEAR_FACTORS = {"um": Decimal("100")}
HITMV_ROTATION_FACTORS = {"degree": Decimal("10000")}
POSITION_DISPLAY_FACTORS = {
    "linear": ("um", Decimal("100")),
    "rotation": ("degree", Decimal("10000")),
}
...
def hitmv_controller_count(value: str, unit: str, stage_kind: str, *, signed: bool = True) -> str:
    ...
    if unit not in factors:
        expected = "degree" if stage_kind == "rotation" else "um"
...
def controller_position_display(raw_position: str, stage_kind: str) -> str:
    return formatted_position_display(raw_position, stage_kind, {})


def formatted_position_display(raw_position: str, stage_kind: str, converted_positions: dict[str, str] | None = None) -> str:
    text = raw_position.strip()
    if not text:
        return ""
    converted_positions = converted_positions or {}
    converted = converted_positions.get(stage_kind, "").strip()
    if converted:
        return f"{text} pls / {converted}"
    unit, factor = POSITION_DISPLAY_FACTORS.get(stage_kind, POSITION_DISPLAY_FACTORS["linear"])
    try:
        raw = Decimal(text)
    except InvalidOperation:
        return text
    computed = raw / factor
    return f"{text} pls / {decimal_plain_text(computed)} {unit}"


def shrc_position_text(value: str, unit: str) -> str:
    text = value.strip()
    if not text:
        return ""
    if text[0] in {"U", "D"}:
...
class AxisState:
    ...
    selected: bool = False
    position: str = ""
    converted_positions: dict[str, str] = field(default_factory=dict)
    ready: str = ""
    detail_status: str = ""
...
class HitMvController(BaseController):
    ...
    def build_jog_speed_overrides(
        ...
        for axis in sorted(set(axes)):
            ...
            slow = hitmv_controller_count(
                slow_by_axis[axis],
                unit,
                axis_kinds.get(axis, "linear"),
                signed=False,
            )
            fast = hitmv_controller_count(
                fast_by_axis[axis],
                unit,
                axis_kinds.get(axis, "linear"),
                signed=False,
            ...
            commands.append(f"D:{axis - 1},{slow},{fast},{rate}")
        return commands
...
class Shrc203Controller(BaseController):
    ...
    def refresh_status(self) -> dict[int, dict[str, str | bool]]:
        ...
        if not self.active_axes:
            self.active_axes = self.detect_active_axes()
        position_fields = split_reply(self.send("Q:"), self.axis_count)
        position_um_fields = split_reply(self.send("Q:U"), self.axis_count)
        position_degree_fields = split_reply(self.send("Q:D"), self.axis_count)
        busy_fields = split_reply(self.send("!:"), self.axis_count)
        ...
        for index in range(self.axis_count):
            ...
            status[axis] = {
                ...
                "converted_positions": {
                    "linear": shrc_position_text(position_um_fields[index], "um") if active else "",
                    "rotation": shrc_position_text(position_degree_fields[index], "degree") if active else "",
                },
                "ready": ready,
                "detail_status": detail,
            }
        ...
        for axis in sorted(set(axes)):
            ...
            commands.append(
                self.build_speed(
                    ...
                    slow_by_axis[axis],
                    fast_by_axis[axis],
                    (rate_by_axis or {}).get(axis, DEFAULT_RATE),
                    unit,
                )
            )
        return commands
...
class CombinedControllerApp(tk.Tk):
    ...
    def _build_axis_table(self, parent: ttk.Frame) -> None:
        ...
        headers = ["Use", "Active", "Controller", "Axis", "Kind", "Position", "Ready/Busy", "Detail Status", "Slow/Jog", "Fast", "Rate", "Speed"]
    ...
    def _add_axis_row(self, row: int, key: str, label: str, axis: int) -> None:
        ...
        labels = [
            ...
            ttk.Label(self.axis_table, textvariable=vars_.position, width=32),
        ...
        kind_combo.bind("<<ComboboxSelected>>", lambda _event, key=key, axis=axis: self._refresh_axis_position_display(key, axis))
    ...
    def _apply_controller_status(
        ...
        for axis in range(1, self.controllers[key].axis_count + 1):
            ...
            row.state.position = str(values.get("position", ""))
            converted = values.get("converted_positions", {})
            row.state.converted_positions = converted if isinstance(converted, dict) else {}
            row.position.set(formatted_position_display(row.state.position, row.stage_kind.get(), row.state.converted_positions))
    ...
    def _refresh_axis_position_display(self, key: str, axis: int) -> None:
        row = self.axis_rows[(key, axis)]
        row.position.set(formatted_position_display(row.state.position, row.stage_kind.get(), row.state.converted_positions))

    def validate_unit_for_axes(self, selected: dict[str, list[int]], unit: str) -> None:
        if unit == "controller":
            return
        kinds = {
            self.axis_rows[(key, axis)].stage_kind.get()
            for key, axes in selected.items()
            for axis in axes
        }
        if len(kinds) > 1:
            raise ValueError("Use controller units when moving mixed linear and rotation axes together.")
        if kinds == {"linear"} and unit != "um":
            raise ValueError("Linear axes require unit 'um' or 'controller'.")
        if kinds == {"rotation"} and unit != "degree":
            raise ValueError("Rotation axes require unit 'degree' or 'controller'.")
    ...
    def _send_grouped_commands(
        ...
        if validate_unit:
            try:
                self.validate_unit_for_axes(selected, self.motion_unit_var.get())
    ...
    def jog_selected(self, direction: str) -> None:
        def builder(controller: BaseController, axes: list[int]) -> list[str]:
            slow_by_axis = {axis: self.axis_rows[(controller.key, axis)].slow.get() for axis in axes}
            ...
            speed_commands = controller.build_jog_speed_overrides(
                axes,
                slow_by_axis,
                fast_by_axis,
                rate_by_axis,
            ...
            return [*speed_commands, jog_command]
    ...
    def set_speed_selected(self) -> None:
        ...
        self._send_grouped_commands("Set Speed", builder, refresh_after=False, validate_unit=True)
    ...
    def set_speed_axis(self, key: str, axis: int) -> None:
        ...
        if not controller.connection.is_open:
            ...
            return
        try:
            self.validate_unit_for_axes({key: [axis]}, self.motion_unit_var.get())
        except ValueError as exc:
            messagebox.showwarning("Set Speed", str(exc))
            return
