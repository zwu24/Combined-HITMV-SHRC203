...
class HitMvController(BaseController):
    ...
    def build_jog_speed_overrides(
        ...
        # HIT-MV vendor sample does not expose a jog override command, so jog
        # follows the controller's current D: speed setting.
        return []
...
class CombinedControllerApp(tk.Tk):
    ...
    def jog_selected(self, direction: str) -> None:
        def builder(controller: BaseController, axes: list[int]) -> list[str]:
            ...
            return [
                controller.build_jog(axes, direction),
                *controller.build_jog_speed_overrides(
                    axes,
                    fast_by_axis,
