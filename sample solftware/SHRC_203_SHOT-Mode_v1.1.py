...
def ensure_shot_fc_format(port):
    port.write('?:FMT\r\n'.encode())
...
def run_relative_move_smoke(axis='2', pulses='100'):
    test_ser = open_controller()
    try:
        if not ensure_shot_fc_format(test_ser):
            return
        for command in (
            '?:FMT\r\n',
            'Q:\r\n',
            'D:' + axis + 'S2000F20000R200\r\n',
            'M:' + axis + '+P' + pulses + '\r\n',
            start_command(axis),
        ):
            print(command)
...
def click_Comm():
    ...
    global ComNumber
    ser = open_controller()
    if not ensure_shot_fc_format(ser):
        ser.close()
        ser = None
        return
